...
argument using the fact that \(O_4^+(2^f)\) meets every involution class.
...
\begin{proof}
For \(k=1\), a regular subgroup on \(X/H\) would itself be core-free and
transitive.  Let \(k\ge2\).  The action in \cref{lem:pa-primitive} is a
...
& not in either torus type or \(P_1\); exceptional factors are checked below \\
\(\PSL_3(q)\), \(q\ne4\), excluding \(q=2\)
& split-torus normalizer
& not in a point or hyperplane parabolic or in the extension-field factor \\
...
& \(P_2\)
& absent from A.1 except \((n,q)=(5,2)\) \\
\(\PSL_n(q)\), odd \(n\ge5\), graph present
& \(P_{2,n-2}\)
& only parabolic overgroups \(P_2,P_{n-2}\); absent except \((5,2)\) \\
\(\PSp_{2m}(q)\), \(m\ge3\)
& \(P_2\)
& absent from A.2 \\
\(\PSp_4(q)\), \(q\) odd
...
\(\PSU_3(q)\)
& \(N_1\)
& absent from the \(n=3\) rows of A.3 \\
\(\PSU_4(q)\), \(q\ge3\)
...
& central-node \(P_2\)
& fixed by triality and absent from A.7 \\
\bottomrule
\end{longtable}
...
It remains to treat \(S=\PSp_4(q)\) with \(q=2^f\), \(f\ge2\).  Let
\(U=\Omega_4^+(q)\) and \(H=N_X(U)\).  Case (10.1) of
\cite[Section 10]{lps-regular} gives that \(H\) is core-free maximal and
\(H\cap S=O_4^+(q)\).  Thus \(X=HS\), and hence
\begin{equation}\label{eq:sp-degree}
\lvert X:H\rvert=\lvert S:O_4^+(q)\rvert=\frac{q^2(q^2+1)}2,\qquad
v_2(\lvert X:H\rvert)=2f-1.
\end{equation}
Corollary 2.3 of \cite{lps-regular} shows that \(O_4^+(q)\) meets every
involution class of \(S\), so \(S\) is 2-elusive on \(X/H\).  Moreover,
the standard outer-automorphism description
\cite[Table 2.1]{xiali-solvable} gives \(|X/S|\mid 2f\), and hence
\[
v_2(|X/S|)\le1+v_2(f)<2f-1.
\]
The last strict inequality follows, for example, from
\(v_2(f)\le f-1\) and \(f\ge2\).
Now \cref{lem:valuation} applies.  This is the sole infinite classical row
for which the factor screen itself does not suffice.
...
cases choose a Borel \(P_1\cap P_2\) in \(G_2(3^f)\), or
\(P_1\cap P_4\) in \(F_4(2^f)\).  Their respective \(p\)-parts are \(q^6\)
...
\cite[Theorems 1.1--1.3]{giudici}.  Comparing those tables with the Atlas
...
TomLib 1.2.11.  The generated TSV has SHA-256
\begin{center}
{\footnotesize
\nolinkurl{82bcf695617014f0124839c5a01983a6c8904fc5bc92e0893c9c2601c43bd3a0}}
\end{center}
...
\cite[Corollary 3]{lps-transitive}.  The involution input for
\(\PSp_4(2^f)\) is \cite[Corollary 2.3]{lps-regular}.  No property of a
...
AI agents were used extensively for literature search, proof exploration,
code generation, drafting, and adversarial review.  The author directed the
research process, selected the arguments and sources included here, reran the
reported computations, and assumes full responsibility for every mathematical
