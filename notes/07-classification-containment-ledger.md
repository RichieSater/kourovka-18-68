...
**Status.**  Exhaustiveness and the audited factorization rows are **published
input**, except that the exact LPS (1990) Theorem D/Remark 2 consequence is an
**explicit source assumption**.  The deductions from those rows are **proved**
here.  The fifteen
...
**Source assumption.** The following large-alternating consequence is
attributed to Liebeck--Praeger--Saxl (1990), Theorem D and Remark 2, but its
exact primary-text match was not reopened in source traces through 2026-08-31.
...
AtlasRep test at \(q=4\) is only a regression certificate for the formerly
...
only exact factorizations, so the factor-free screen uses Tables 1--3;
...
| Coordinate \(X\) | Giudici factorization locus | Chosen maximal \(H\) | Finite-certificate status |
|---|---|---|---|
...
| \(He.2\) | Table 3, \(\operatorname{Aut}(He)\) block, plus lifts of Table 2 | \(2^2.L_3(4).D_{12}\) | published screen only |
| \(Ru\) | Table 2, \(Ru\) block | \(2^6{:}U_3(3){:}2\) | published screen only |
| \(Suz\) | Table 2, \(Suz\) block | \(J_2.2\) | published screen only |
| \(Suz.2\) | Table 3, \(\operatorname{Aut}(Suz)\) block, plus lifts of Table 2 | \(J_2.2\times2\) | published screen only |
| \(Fi_{22}\) | Table 2, \(Fi_{22}\) block | \(O_7(3)\) | published screen only |
| \(Fi_{22}.2\) | Table 2, \(Fi_{22}\) block, lifted by Lemma 2.1(5) | \(G_2(3).2\) | published screen only |
| \(Co_1\) | Table 2, \(Co_1\) block | \(2^{11}{:}M_{24}\) | published screen only |
