# Classification and containment ledger

**Status.**  Exhaustiveness and the listed factorization rows are **published
input**.  The deductions from those rows are **proved** here.  The fifteen
small Table-of-Marks closeouts are **computationally certified**.  This ledger
records the exact audit promised in the manuscript; it is not a replacement
for the cited classification theorems.

## 1. Audit contract

The factor-screen lemma starts with a proper nontrivial subgroup \(V<S\)
whose \(S\)-class is invariant under the coordinate group
\(S\le X\le\operatorname{Aut}(S)\).  If a core-free subgroup is transitive
on the resulting cosets, then some nontrivial maximal factorization
\(Y=AB\), \(S\le Y\le X\), has \(V\le A\cap S\) after exchanging the
factors.  Therefore every entry below answers four separate questions:

1. **exhaustive locus:** which theorem and exact table row lists the possible
   factor intersection;
2. **containment:** why that intersection cannot contain \(V\), not merely
   why it is not visibly equal to \(V\);
3. **outer behavior:** why the \(S\)-class of \(V\) survives every allowed
   diagonal, field, graph, or triality automorphism in \(X\); and
4. **maximal action:** where maximality of the point stabilizer comes from.

For the ordinary screen, the last point is elementary: class invariance gives
\(N_X(V)S=X\), and any maximal overgroup \(H<X\) of \(N_X(V)\) is core-free
and supplements \(S\).  Thus \(V\) itself need not be maximal.  When \(V\)
is a maximal subgroup of \(S\), containment in a proper factor intersection
forces equality.  This observation applies to every standard maximal
parabolic and every full decomposition stabilizer below.
...
**Exhaustive source.** Liebeck--Praeger--Saxl (1990), Theorem D and Remark 2.
For \(n\ge13\), any intransitive factor in a nontrivial maximal
factorization stabilizes a \(j\)-set with \(j\le5\); the only stated
exceptions have degrees 6, 8, or 10.  The chosen 6-set stabilizer is maximal
because \(6<n/2\), is invariant under \(S_n=\operatorname{Aut}(A_n)\), and
cannot be one of those factors.
...
| A.1 row | Published factor intersections | Screen and containment disposition |
|---:|---|---|
...
| 9 | \(L_2(7)\) versus \(A_6\), \(n=3,q=4\) | All ten coordinate groups with socle \(L_3(4)\) are closed individually by complete Table-of-Marks certificates. |
| 10 | \(31{:}5\) versus \(P_2\) or \(P_3\), \(n=5,q=2\) | This is the allowed socle \(L_5(2)\).  It is precisely the low parameter at which the odd-dimensional parabolic screen is not asserted to work. |

#### 3.2 The two torus screens
...
For \(m\ge3\), the screen is the maximal parabolic \(P_2\).  For \(m=2\)
and odd \(q\), it is the full stabilizer of a nonsingular \(2+2\)
...
| 5 | \(Sp_m(4).2\) versus \(N_2\), \(m\ge4,q=2\) | The nonsingular stabilizer \(N_2\) is not the parabolic \(P_2\); neither factor equals the maximal screen. |
...
The screens are \(N_1\) for \(n=3\), the full nonsingular \(2+2\)
...
In every row here the screen is the maximal parabolic \(P_2\), invariant
...
nonsingular/decomposition screens above are in Kleidman--Liebeck, Chapters
...
\(r\) is prime.  Harper, Lemma 2.1, gives
\(\operatorname{Aut}(S)=\langle S,\rho\rangle\), with
\(\rho^2=\varphi\).  Thus \(\rho\) commutes with Frobenius and normalizes
...
\[
v_2(|X:H|)=v_2(|S:V|)=4(f-d),\qquad
v_2(|X:S|)\le1+v_2(f)<2f\le4(f-d).
\]

The socle-valuation lemma closes every exponent and every coordinate group.
This is not a claim extracted from a finite computation.  The separate
AtlasRep test at \(q=4\) is only a regression certificate for the referee's
counterexample to the former \(O_4^+\)-normalizer construction.
...
graph-stable screens.  The small derived-group isomorphisms
\(G_2(2)'\cong U_3(3)\) and \({}^2G_2(3)'\cong L_2(8)\) route to the
classical audit.

## 5. Sporadic socles

**Exhaustive source.** Giudici, Theorems 1.1--1.2 and Tables 1--4.
Tables 1--3 cover factorizations of a simple sporadic socle itself.  Table 4
lists the genuinely new outer-extension cases; Lemma 2.1(5) lifts the
applicable simple-socle rows to an outer extension.
Maximality of each displayed \(H\) was cross-checked against the *Atlas of
Finite Groups* maximal-subgroup list (and, where available, the official
online ATLAS list).  For each factorable coordinate group, the applicable
...
| Coordinate \(X\) | Exact Giudici locus | Chosen maximal \(H\) | Independent finite status |
|---|---|---|---|
...
| \(M_{22}.2\) | Table 4, \(\operatorname{Aut}(M_{22})\) block | \(A_6.2^2\) | TomLib cross-check |
| \(M_{23}\) | Table 2, \(M_{23}\) block | \(A_8\) | TomLib cross-check |
| \(J_2\) | Table 1, \(J_2\) block | \(3.A_6.2_2\) | TomLib cross-check |
| \(J_2.2\) | Table 4, \(\operatorname{Aut}(J_2)\) block | \(3.A_6.2^2\) | TomLib cross-check |
| \(HS\) | Table 2, \(HS\) block | \(L_3(4).2_1\) | TomLib cross-check |
| \(HS.2\) | Table 4, \(\operatorname{Aut}(HS)\) block, plus lifts of Table 2 | \(L_3(4).2^2\) | TomLib cross-check |
| \(He\) | Table 2, \(He\) block | \(2^2.L_3(4).S_3\) | published screen only |
| \(He.2\) | Table 4, \(\operatorname{Aut}(He)\) block, plus lifts of Table 2 | \(2^2.L_3(4).D_{12}\) | published screen only |
| \(Ru\) | Table 2, \(Ru\) block | \(2^6{:}U_3(3){:}2\) | published screen only |
| \(Suz\) | Table 2, \(Suz\) block | \(J_2.2\) | published screen only |
| \(Suz.2\) | Table 4, \(\operatorname{Aut}(Suz)\) block, plus lifts of Table 2 | \(J_2.2\times2\) | published screen only |
| \(Fi_{22}\) | Table 3, \(Fi_{22}\) block | \(O_7(3)\) | published screen only |
| \(Fi_{22}.2\) | Table 3, \(Fi_{22}\) block, lifted by Lemma 2.1(5) | \(G_2(3).2\) | published screen only |
| \(Co_1\) | Table 3, \(Co_1\) block | \(2^{11}{:}M_{24}\) | published screen only |
...
is factor-free.  This is a use of the exhaustive theorem, not of TomLib's
coverage.

The one factor-cover survivor is \(M_{24}\), whose Table 2 block contains the
